...
function checkAccess() {
  if (!window.LAB_USER) {
    showLockScreen(`Доступ только для участников группы.<br><br>Войдите через <a href="https://apparchi.ru" style="color:#aaa">apparchi.ru</a>`);
    return false;
  }
  return true;
}
...
/* ============================================================
   1. Водяной знак (использует ID из LAB_USER)
   ============================================================ */

(function () {
  if (window.LAB_USER?.id) {
    const wm = document.getElementById("watermark");
    const viewer = document.getElementById("viewerWrapper");

    if (wm && viewer) {
      const id = String(window.LAB_USER.id);
      let raf = null;
